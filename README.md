...
отдельные semantic и lexical режимы доступны для диагностики. Генерация
RAG-ответа, rule engine, Telegram, n8n, Docker Compose, подтверждение и
регистрация заявки на этом этапе не подключены.
...
6. В Project Settings найдите URL проекта и backend service role key.
7. Создайте локальный `.env`:
...
генерация LLM/RAG-ответа, rule engine для числовых диапазонов, reranker и
