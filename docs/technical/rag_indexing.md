...
UUID строится из `document_id`, пути раздела и нормализованного содержимого, но
не из `chunk_index`. Поэтому при вставке нового раздела прежний content-based
UUID может переместиться на следующую позицию и временно быть занят строкой со
старым `chunk_index`. Перед основным upsert индексатор переносит такие строки на
свободные временные позиции по `id`, не удаляя их и не сбрасывая неизменившиеся
embeddings. Затем актуальные строки занимают свои natural keys, а обычный stale
cleanup выполняется только после успешной индексации всего набора. Повторный
запуск после частично выполненного переноса идемпотентен.
