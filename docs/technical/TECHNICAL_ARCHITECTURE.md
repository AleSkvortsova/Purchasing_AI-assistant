...
Если content-based UUID перемещается между позициями после вставки раздела,
занимающие его старые строки сначала недеструктивно переносятся на свободные
временные позиции. Это исключает PK collision до stale cleanup и позволяет
безопасно повторить индексацию после частичного сбоя.
